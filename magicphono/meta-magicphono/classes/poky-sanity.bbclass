...
python poky_update_bblayersconf() {
    current_version = int(d.getVar('MAGICPHONO_BBLAYERS_CONF_VERSION', True) or -1)
    latest_version = int(d.getVar('REQUIRED_MAGICPHONO_BBLAYERS_CONF_VERSION', True) or -1)
    ...
    if success:
        ...
        #sanity_conf_update(bblayers_fn, lines, 'MAGICPHONO_BBLAYERS_CONF_VERSION', current_version)
        index, line = sanity_conf_find_line(r'^MAGICPHONO_BBLAYERS_CONF_VERSION', lines)
        lines[index] = 'MAGICPHONO_BBLAYERS_CONF_VERSION = "%d"\n' % current_version
...
BBLAYERS_CONF_UPDATE_FUNCS += "conf/bblayers.conf:MAGICPHONO_BBLAYERS_CONF_VERSION:REQUIRED_MAGICPHONO_BBLAYERS_CONF_VERSION:poky_update_bblayersconf"
