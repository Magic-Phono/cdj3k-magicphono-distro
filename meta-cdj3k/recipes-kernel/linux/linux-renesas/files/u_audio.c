...
#ifdef CONFIG_PDJ
static int uac_pcm_rate_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
	uinfo->value.integer.min = 44100;
	uinfo->value.integer.max = 96000;
	return 0;
}

static int uac_pcm_rate_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
#ifdef CONFIG_PDJ
static struct snd_kcontrol *u_audio_get_ctl(struct g_audio *audio_dev, const char *name)
{
...
int u_audio_set_capture_srate(struct g_audio *audio_dev, int srate)
{
	...
	struct uac_params *params = &audio_dev->params;
struct uac_rtd_params *prm = &audio_dev->uac->p_prm;
unsigned int *p_rbuf = (unsigned int*)prm->rbuf;
	int i;
	...
	for (i = 0; i < UAC_MAX_RATES; i++) {
		if (params->c_srate[i] == srate) {
			...
			snd_ctl_notify(audio_dev->uac->card, SNDRV_CTL_EVENT_MASK_VALUE, &ctl->id);
			return 0;
		}
					...
					const char *card_name)
{
	...
	for (i = 0; i < ARRAY_SIZE(uac_pcm_controls); i++) {
		err = snd_ctl_add(card, snd_ctl_new1(&uac_pcm_controls[i], uac));
		if (err < 0)
			goto snd_fail;
	}
#endif
