...
extern int mdiobb_read(struct mii_bus *bus, int phy, int reg);
extern int mdiobb_write(struct mii_bus *bus, int phy, int reg, u16 val);
/* MDC/MDIO, redefine/implement the following Macro */
#define MDC_MDIO_WRITE(bus, phyID, regID, data) \
        mdiobb_write((struct mii_bus*)bus, phyID, regID, data)
#define MDC_MDIO_READ(bus, phyID, regID) \
        mdiobb_read((struct mii_bus*)bus, phyID, regID)
struct mii_bus *copy_mii_bus = NULL;
...
void smi_set_bus(struct mii_bus *bus)
{
    copy_mii_bus = bus;
}

struct mii_bus* smi_get_bus(void)
{
    return copy_mii_bus;
}
...
rtk_int32 smi_read(struct mii_bus *bus, rtk_uint32 mAddrs, rtk_uint32 *rData)
{
#if (!defined(MDC_MDIO_OPERATION) && !defined(SPI_OPERATION))
    ...
    *rData = *rData & 0x0000FFFF;
    ...
    MDC_MDIO_WRITE(bus, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL0_REG, MDC_MDIO_ADDR_OP);

    /* Write address to register 23 */
    MDC_MDIO_WRITE(bus, MDC_MDIO_PHY_ID, MDC_MDIO_ADDRESS_REG, mAddrs);

    /* Write read control code to register 21 */
    MDC_MDIO_WRITE(bus, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL1_REG, MDC_MDIO_READ_OP);

    /* Read data from register 25 */
    *rData = MDC_MDIO_READ(bus, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_READ_REG);
...
rtk_int32 smi_write(struct mii_bus *bus, rtk_uint32 mAddrs, rtk_uint32 rData)
{
#if (!defined(MDC_MDIO_OPERATION) && !defined(SPI_OPERATION))
    rtk_int8 con;
    rtk_uint32 ACK;
    ...
    MDC_MDIO_WRITE(bus, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_WRITE_REG, rData);

    /* Write data control code to register 21 */
    MDC_MDIO_WRITE(bus, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL1_REG, MDC_MDIO_WRITE_OP);
